...
namespace rpiApp.Views
{
    ...
    public partial class TextEditView : UserControl
    {
        ...
        private int _currentTheme = (int)ThemeName.HighContrastDark;
        ...
        public TextEditView()
        {
            ...
            Language language = _registryOptions.GetLanguageByExtension(".py");
            ...
            //_syntaxModeCombo.SelectedItem = language;
            ...
            //var vm = new MainWindowViewModel(_textMateInstallation, _registryOptions);
            var vm = Ioc.Default.GetRequiredService<TextEditViewModel>();
            //if (vm.TextMateInstallation is null)
            {
                vm.TextMateInstallation = _textMateInstallation;
                vm.RegistryOptions = _registryOptions;
                foreach (ThemeName themeName in Enum.GetValues<ThemeName>())
                {
                    var themeViewModel = new ThemeViewModel(themeName);
                    vm.AllThemes.Add(themeViewModel);
                    if (themeName == ThemeName.HighContrastDark)
                    {
